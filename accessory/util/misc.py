...
def _get_latest_checkpoint_path(ckpt_path):
    subfolders = [f for f in os.listdir(ckpt_path) if os.path.isdir(os.path.join(ckpt_path, f))]
    # subfolder name should be epochx-iterxxxx, sort 
    iter_pattern = re.compile(r'^epoch(\d+)-iter(\d+)$')
    epoch_pattern = re.compile(r'^epoch(\d+)$')
    
    parsed_folders = []
    for folder in subfolders:
        iter_match = iter_pattern.match(folder)
        epoch_match = epoch_pattern.match(folder)
        
        if iter_match:  # The folder name has the structure epochX-iterY
            epoch, iteration = map(int, iter_match.groups())
            parsed_folders.append((epoch, iteration, folder))
        elif epoch_match:  # The folder name has the structure epochX
            epoch = int(epoch_match.group(1))
            # We assign a high iteration number to make sure complete epochs are considered latest
            parsed_folders.append((epoch, float('inf'), folder))

    # Sort the folders by epoch first and then by iteration
    parsed_folders.sort(key=lambda x: (x[0], x[1]), reverse=True)

    # The latest folder is the first one in the sorted list (if there are any valid folders)
    latest_folder = parsed_folders[0][2] if parsed_folders else None
...
def resume_stage1(args, model_without_FSDP):
    ...
    """
    if args.resume:
        if "epoch" not in args.resume or "iter" not in args.resume:
            latest_folder = _get_latest_checkpoint_path(args.resume)
            if latest_folder:
                args.resume = os.path.join(args.resume, latest_folder)
            else:
                print(f"no valid checkpoint found in {args.resume}")
                return
