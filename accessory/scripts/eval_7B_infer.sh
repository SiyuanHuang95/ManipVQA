remove_space=True
pretrained_path=PATH_TO_ManipVQA7B
addition_flag=internlm7b
...
tokenizer_path="PATH_TO_ManipVQA7B/tokenizer.model"
...
sample_num=2011
vqa_data=PATH/demo_data/demo.json
srun -p  test_s2 -N 1 --gres=gpu:1 --quotatype=auto  --job-name=${vqa_data} \
torchrun --nproc-per-node=1 --master_port=$PORT eval_manip_7B_single_gpu.py \
    --llama_type ${MODEL} \
